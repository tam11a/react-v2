...
const getInterestedCustomerById = (id?: string) => {
	return instance.get(`/assets/${id}/interested-buyers`);
};

export const useGetInterestedCustomerById = (id?: string) => {
	return useQuery(
		["get-all-interested-customer-by-id", id],
		() => getInterestedCustomerById(id),
		{
			enabled: !!id,
		}
	);
